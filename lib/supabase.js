...
async function recordCommentTrigger({ instagramUserId, clientId, commentId, commentText, keyword, mediaId }) {
  const marker = `comment:${commentId}`;
  const message = JSON.stringify({
    type: 'comment_keyword_trigger',
  ...
  if (error) {
    if (error.code === '23505') return { status: 'duplicate', marker };
    throw new Error(`[Supabase] recordCommentTrigger: ${error.message}`);
  }

  return { status: 'ok', marker };
}

async function recordCommentPrivateReply({ instagramUserId, clientId, commentId, replyText, messageId }) {
  const marker = messageId ? `comment_reply:${messageId}` : `comment_reply:${commentId}`;
  const message = JSON.stringify({
