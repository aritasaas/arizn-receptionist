...
async function getActiveClients() {
  const { data, error } = await db().from('clients').select('*').eq('active', true);
...
async function saveInstagramConnection({
  ...
  if (lookupError) throw new Error(`[Supabase] saveInstagramConnection lookup: ${lookupError.message}`);
  ...
  if (existing) {
    ...
    if (error) throw new Error(`[Supabase] saveInstagramConnection update: ${error.message}`);
    return data;
  }
  ...
  if (error) throw new Error(`[Supabase] saveInstagramConnection insert: ${error.message}`);
  return data;
}
